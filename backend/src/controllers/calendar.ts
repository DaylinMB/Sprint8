...
import EventsCalendar from '../models/calendar';

export const createEvent = async (req: Request, res: Response) => {
const { title, start, end } = req.body;
try {
   const event = await EventsCalendar.create({ title, start, end });
   res.json(event);
} catch (error) {
...
try {
   const events = await EventsCalendar.findAll();
   res.json(events);
} catch (error) {
...
const { id } = req.params;
try {
   const event = await EventsCalendar.findByPk(id);
   if (!event) {
